...
class HandoffCommitsItsOwnOutputTest(HandoffBuildContractTest):
    """Export demands a clean tree, and generating the handoff is what dirties it."""
    ...
    def test_committing_leaves_the_tree_clean_for_export(self) -> None:
        ...
        build_handoff(root, "project/handoff-build", generated_at=self.generated_at,
                      research_commit=self.commit, commit=True)

        self.assertEqual("", self._git(root, "status", "--porcelain"),
                         "the generated handoff must be committed, or export cannot prove the commit")
    ...
    def test_unrelated_changes_are_not_swept_into_the_commit(self) -> None:
        root, project = self._repository()
        stray = root / "projects" / "handoff-build" / "00_intake" / "constraints.yaml"
        ...
        self.assertIn("constraints.yaml", remaining,
                      "another agent's edit must stay uncommitted, or the bundle points at a commit it did not mean")
