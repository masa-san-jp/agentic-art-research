...
1. 評価軸ごとの得点と、その点にした理由を**全候補について書き切る**。書く前に選ばない。
2. それでも同点なら、**棄却したときに失うものが小さい方**を採る。理由に「何を失うか」を両方書く。
3. なお決まらないなら、その同点自体を `uncertainty-register.yaml` に登録し、比較を `HUMAN_SELECTION_REQUIRED` にする。**曖昧なまま推薦しない。**
