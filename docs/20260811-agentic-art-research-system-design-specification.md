...
- [ ] 本書を正本としたディレクトリ構造が存在する。
- [ ] 主要7スキーマが実装されている。
- [ ] `new_project.py` が完全な雛形を生成する。
- [ ] `validate.py --check` が正常系・異常系テストを通る。
- [ ] `build_graph.py` が証拠から要件までの依存グラフを生成する。
- [ ] `bundle.py` が人間向けと制作エージェント向けの2種類を生成する。
- [ ] `impact.py` が証拠IDから影響する要件を列挙する。
- [ ] GitHub Actionsが検証と生成物鮮度を確認する。
- [ ] サンプルプロジェクト1件が `COMPLETE` または `COMPLETE_WITH_GAPS` へ到達する。
- [ ] 生の個人データをGitへ入れずに個人証拠由来の派生シグナルを利用できる。
